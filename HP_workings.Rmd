...
hp3 <- hp3 %>% group_by(desc,sale_date) %>% 
...
hp3_dub_dyg <- dygraph(hp3_ts, group = "nh_sh", main = "Dublin New Houses vs Second Hand Houses") %>% 
...
hp4_oth_dyg <- dygraph(hp4_ts, group = "nh_sh", main = "Other Towns New Houses vs Second Hand Houses") %>% 
  dyOptions(labelsUTC = TRUE, fillGraph=TRUE, fillAlpha=0.1, drawGrid = TRUE) %>%
  dyRangeSelector() %>%
  dyCrosshair(direction = "vertical") %>%
  dyHighlight(highlightCircleSize = 4, 

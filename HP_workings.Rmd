...
```{r}
hp6 <- hp %>% filter(desc == 'New Dwelling house /Apartment') %>% group_by(county,desc) %>% summarise(n=length(county),median_price=median(price_adj))

hp6 %>% ggplot(mapping=aes(x=median_price, y=n)) + geom_point(aes(size=n))+
  geom_text(data = filter(hp6, n >5000),aes(label=county),hjust = 0.5, nudge_x = 0.5,vjust = -0.7, nudge_y = 0.5)+
  labs(title="Average Price vs Number of Sales for New Homes",
        x ="Average Price", y = "Number of Sales")

hp7 <- hp %>% filter(desc == 'Second-Hand Dwelling house /Apartment') %>% group_by(county,desc) %>% summarise(n=length(county),median_price=median(price_adj))

hp7 %>% ggplot(mapping=aes(x=median_price, y=n)) + geom_point(aes(size=n))+
  geom_text(data = filter(hp7, n >18000),aes(label=county),hjust = 0.5, nudge_x = 0.5,vjust = -0.7, nudge_y = 0.5)+
  labs(title="Average Price vs Number of Sales for Second-Hand Homes",
        x ="Average Price", y = "Number of Sales")
```

```{r}
# #Finding Costliest properties in each county per year
hp6 <- aggregate(hp$price_adj, by = list(hp$county, hp$year), max)
...
```{r}
# #Findng Cheapest properties in each county per year
min(hp$price)

hp7 <- aggregate(hp$price_adj, by = list(hp$county,hp$year), min)

names(hp7) <- c("Year","County","Min")

head(hp7)
```

```{r}
#Time series plot for new homes
```

```{r}
#Time series plot for second-hand homes
```

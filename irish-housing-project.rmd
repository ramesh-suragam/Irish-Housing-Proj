...
names(hp5) <- c("desc","sale_date","count")

# hp5 <- hp5 %>% group_by(desc,sale_date) %>% summarise(count = median(price_adj))

hp5 <- spread(hp5, desc, count) 
...
names(hp6) <- c("desc","sale_date","count")

# hp6 <- hp6 %>% group_by(desc,sale_date) %>% summarise(count = median(price_adj))

hp6 <- spread(hp6, desc, count) 

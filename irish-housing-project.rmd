...
p <- ggplot(change, aes(x=year, y=hprice, color=town_f))+
  ...
  labs(title="Fig. 2b: Property price change for towns within different regions of Ireland (2010-2020)",
...
In general, primary towns, and moreso, towns within 10 km of these, tended to have the highest median property prices across all years and regions (Fig. 2a, 2b). Noticeable exceptions include Maynooth (Eastern) and Slane (North East), where both exceeded the median property price of the primary and it's next closest town, for at least half of the time period (Fig. 2b). This could be due to the fact that the proportion of houses to apartments increases further out from urban areas, with houses typically selling at a higher price than apartments. Both towns are also popular commuter towns, with demand pushing up prices in these areas. The North showed very little differentiation between it's four towns (Fig. 2a, 2b). The Northern region, along with the Midlands and North West (excluding Strandhill), had the lowest median property prices of all the regions (Fig. 2a); the Eastern region had the highest (Fig. 2a, 2b). Most towns saw a drop in median property prices from 2010 to 2012/2013, with the majority seeing a recovery and upwards trend from 2014 onwards (Fig. 2b). 
...
```{r, fig.width=9, fig.height= 6,eval=T, echo=FALSE, fig.cap="Fig 3: Heatmap of property prices in Dublin commuter belt (2010-2019)", fig.align="center"}
...
```{r echo=FALSE, fig.height = 6, fig.width = 12, fig.cap = "Fig 4: Median property price change for costal towns", fig.align = "center"}
...
```{r echo=FALSE,fig.height = 5, fig.width = 10, fig.cap = "Fig 5: Median property price change for costal towns: New vs Second-hand", fig.align = "center"}
...
When it comes to house buying there is a huge dilemma whether to buy a new property or a second-hand property and whether to buy it in or outside of Dublin. The adjacent <b>interactive</b> plot(Fig 6.) helps us in making this decision by understanding the trends across the last ten years.
...
```{r}
htmltools::tags$figcaption("Fig 6: Interactive plot for comparision between New and Second-hand properties between Dublin & Other Towns", style = "float:right;width: 450px; padding: 10px 0 0 0; text-align:center; font-size: 0.85em; font-weight: bold")
```
...
```{r, fig.cap="Fig. 7: Median property price change for towns based on county and region", fig.align= "center"}
...
```{r, fig.cap="Fig. 8: Rate of price change over years for major towns", fig.align= "center"}
...
```{r, fig.cap="Fig. 9: Average property price vs Total sale price for four towns", fig.align= "center"}
